...
\textbf{Reality check:} whether the \code{[AI]} line contains any visible text depends on the model. The system prompt asks for brief reasoning before each action, but in observed runs Gemini Flash Lite ignores that and returns an \emph{empty} text with just the tool call --- so the real conversation is mostly \code{[AI] "" -> press\_buttons(...)}. Stronger models (or a mandatory ``reasoning'' parameter added to the tool schema) produce the explanations shown above.
