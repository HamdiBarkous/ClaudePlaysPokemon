...
`- src/pokemon_agent/
   ...
   |  |- tools/navigate_to.py       <- auto-walking tool (A* over the explored map)
...
\code{emulator.py} & Knows how to press buttons, take screenshots, and maintain the explored world map. \\
...
\subsection{The Screenshots (Per-Press Keyframes)}

PyBoy exposes the current screen as a $160\times144$ pixel image. The emulator captures one \textbf{keyframe after each button press settles}, so a batched action (say, five A-presses through a dialog) yields five screenshots --- the model sees everything that happened during its action, not just the end state (the last \code{MAX\_KEYFRAMES} are kept, the final one always being the current state). Each frame is encoded as a \textbf{base64 data URL} --- the image converted into a long text string like \code{data:image/png;base64,iVBORw0KGg...} --- because LLM APIs only accept text/JSON. Token cost per frame is controlled by the \code{MEDIA\_RESOLUTION} setting (Gemini-only; \code{low} $\approx$ 255 tokens/frame reads Game Boy text perfectly, verified empirically).
...
\subsection{The Explored World Map (Overworld Only)}

The model has no spatial memory of its own: each turn it only sees the current screen, so left alone it re-explores the same rooms and circles mazes for hours (every public ``LLM plays Pokémon'' project hit this). So the program keeps a persistent \textbf{fog-of-war map}: after every settled button press, the walkability of every tile currently on screen is recorded into a per-map dictionary, in the game's own coordinate system. The observation then includes a rendering of everything explored so far:

\begin{verbatim}
VIRIDIAN POKECENTER - explored map. You are at (x=3, y=7), facing up.
      0    5
 y=3 #....S....
 y=4 ##........
 y=5 ##........
...
D door/warp, S person/object standing there right now, arrow = you.
Doors/warps on this map: (3, 7), (4, 7)
People/objects on screen: (4, 3)
Reachable UNEXPLORED spots: (3, 3) 4 steps, (2, 3) 5 steps, ...
\end{verbatim}

Three parts matter:
\begin{itemize}[nosep]
  \item \textbf{The map itself} remembers terrain far off screen, with coordinate rulers so the model can name any tile.
  \item \textbf{Doors/warps} come straight from the map header in RAM (\code{wNumberOfWarps} at \code{0xD3AE}) --- ground truth for where the exits are.
  \item \textbf{The frontier list} (``reachable UNEXPLORED spots'') is computed by flood-filling the explored map: walkable tiles that border unknown space. This is the single most effective anti-stuck signal reported by the Gemini Plays Pokémon project --- when the model feels lost, the prompt tells it to go to one of these instead of claiming it is stuck.
\end{itemize}

The screen is read through PyBoy's ``game wrapper'' ($18\times20$ tiles, shrunk to $9\times10$ walkable squares since the player occupies $2\times2$ tiles) and anchored to the player's RAM coordinates. Recording is skipped whenever the screen is not a clean overworld view (dialogs and menus overwrite the tiles they cover; mid-warp the RAM is half-updated). Tiles under NPCs stay unknown --- NPCs move. The map is saved as \code{states/game-NNN.map.json} next to each save state, so resuming a game resumes its exploration memory too.
...
\begin{verbatim}
{
  ...
  "map": "VIRIDIAN CITY - explored map. You are at (x=23, y=28)..."
}
\end{verbatim}
\textbf{plus one screenshot per button press, as actual images in press order} (see Section~\ref{sec:conversation} --- this is the VisionToolNode trick).

\subsection{Tool 2: \code{navigate\_to} (On by Default)}

Instead of pressing one arrow at a time, the model can say ``walk me to $(23, 25)$'' --- using the same coordinates the explored map, the doors list, and the frontier list are labeled with. The program runs \textbf{A*} over the fog-of-war map (not just the visible screen), so it can walk to places far off screen, then executes the path one verified step at a time:

\begin{enumerate}[nosep]
  \item Plan a path through explored, walkable tiles to the target.
  \item Press one step; check the player's RAM position actually changed.
  \item If it didn't (a wandering NPC, a ledge, a spin tile --- things collision data can't see), mark that step as blocked and re-plan around it.
  \item Stop early --- with an explanation to the model --- when a warp triggers (``walked through a door into a new area''), when a dialog or battle interrupts, or when no route through explored terrain exists (``explore toward it first'').
\end{enumerate}

Targeting a door walks through it; targeting a person walks up and faces it (the final ``bump'' is how you talk to NPCs). Disable with \code{USE\_NAVIGATOR=false} in your \code{.env}.

\textbf{Why is this safe to leave on now?} The old version could only path within the visible $9\times10$ window, which made it nearly useless. Long-range navigation over remembered terrain is the feature that, per the public Gemini-vs-Claude comparison, let Gemini get twice as far on a third of the actions.
...
\textbf{\code{tools}} --- actually executes the tool call (presses the buttons), captures the observation (screenshot + memory + explored map), and appends it to the conversation as a ``tool result'' message.
...
\code{MAX\_KEYFRAMES} & 8 & per-press screenshots attached to an observation (most recent kept) \\
\code{MEDIA\_RESOLUTION} & low & Gemini image token spend per frame (low$\approx$255, high$\approx$1085) \\
\code{USE\_NAVIGATOR} & true & expose the A* \code{navigate\_to} tool \\
...
\begin{itemize}
  ...
  \item \textbf{Its spatial memory is only as good as its exploration}: the fog-of-war map and \code{navigate\_to} cover everything it has \emph{seen}, but cross-town trips through never-visited territory still mean exploring tile by tile, guided by the frontier list and its imperfect world knowledge of Pokémon Red.
